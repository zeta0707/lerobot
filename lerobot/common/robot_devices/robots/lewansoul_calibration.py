...
def run_arm_auto_calibration(arm: MotorsBus, robot_type: str, arm_name: str, arm_type: str):
    if robot_type == "lx16a":
        return run_arm_auto_calibration_lx16a(arm, robot_type, arm_name, arm_type)
    else:
        raise ValueError(robot_type)


def run_arm_auto_calibration_lx16a(arm: MotorsBus, robot_type: str, arm_name: str, arm_type: str):
    """All the offsets and magic numbers are hand tuned, and are unique to SO-100 follower arms"""
    if (arm.read("Torque_Enable") != TorqueMode.DISABLED.value).any():
        raise ValueError("To run calibration, the torque must be disabled on all motors.")

    if not (robot_type == "lx16a" and arm_type == "follower"):
        raise NotImplementedError("Auto calibration only supports the follower of lx16a arms for now.")
...
def run_arm_manual_calibration(arm: MotorsBus, robot_type: str, arm_name: str, arm_type: str):
    ...
    Example of usage:
    ```python
    run_arm_calibration(arm, "lx16a", "left", "follower")
    ```
    """
    if (arm.read("Torque_Enable") != TorqueMode.DISABLED.value).any():
        raise ValueError("To run calibration, the torque must be disabled on all motors.")
